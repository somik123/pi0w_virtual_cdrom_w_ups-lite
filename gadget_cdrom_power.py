...
ALL_MODES = [MODE_CD, MODE_HDD, MODE_USB, MODE_POWER, MODE_SHUTDOWN]
...
class UPS():
        ...
        def read_temp(self):
                import os
                stream = os.popen('/usr/bin/vcgencmd measure_temp')
                temp = stream.read()
...
class Display:
    ...
    def refresh(self, state):
        ...
        if state.get_mode() == MODE_POWER:

            ups_lite = UPS()
            voltage = ups_lite.read_voltage()
            voltage = math.ceil(voltage*100) / 100
            ...
            draw.text((0,0), mode_text + ": " + str(capacity) + "%", font = self._font)
            draw.text((0,15), "Voltage: " + str(voltage) + "V", font = self._font)
            draw.text((0,30), "State: " + str(state), font = self._font)
            draw.text((0,45), "Temp: " + temp, font = self._font)
